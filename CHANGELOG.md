...
- **Editor de checkout**: ao remover todas as provas sociais (avaliações) ou todos os banners e salvar, a configuração passa a persistir corretamente (antes o merge no servidor mantinha itens antigos quando a lista era enviada vazia).
- **Member Builder (aba Gamificação)**: adicionado botão **Salvar** para persistir ativação/desativação da gamificação e demais alterações da aba, alinhado às outras seções do builder.
- **Área de membros**: mensagem de conteúdo liberado **“Disponível em X dias”** passa a exibir número inteiro e texto adequado (“1 dia” / “N dias”), em vez de decimais longos causados pelo cálculo de dias no Carbon 3.
- **Área de membros**: o **logout** do aluno redireciona para o **login da própria área** (`/m/{slug}/login` ou `/login` em domínio/subdomínio dedicado), e não mais para o login principal da plataforma.
