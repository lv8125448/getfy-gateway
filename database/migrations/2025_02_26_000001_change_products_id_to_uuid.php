...
return new class extends Migration
{
    ...
    /**
     * Converte products.id (bigint) e FKs para UUID — necessário em instalações PostgreSQL,
     * onde a lógica MySQL acima não roda.
     */
    private function runUpPgsql(): void
    {
        $col = DB::selectOne(
        ...
        if ($col) {
            $t = strtolower((string) $col->data_type);
            if ($t === 'uuid') {
                return;
            }
            if (in_array($t, ['character varying', 'character'], true) && (int) ($col->character_maximum_length ?? 0) === 36) {
                return;
            }
        }
        ...
        DB::statement('ALTER TABLE products DROP CONSTRAINT IF EXISTS products_pkey CASCADE');

        DB::statement('ALTER TABLE products DROP COLUMN IF EXISTS id');
        DB::statement('ALTER TABLE products RENAME COLUMN id_new TO id');
        ...
        foreach (self::PRODUCT_FK_TABLES as $table => $columns) {
            ...
            foreach ($columns as $column) {
                ...
                if (! Schema::hasColumn($table, $column.'_new')) {
                    Schema::table($table, function (Blueprint $t) use ($column) {
                        $t->char($column.'_new', 36)->nullable();
                    });
                }
                ...
                DB::statement('ALTER TABLE '.$table.' RENAME COLUMN '.$column.'_new TO '.$column);
                if (! $nullable) {
